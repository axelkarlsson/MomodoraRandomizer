...
internal static class PointerUtility {
    ...
    ///     <typeparamref name="IntPtr.Zero"/> otherwise
    /// </returns>
    public static IntPtr CreatePointer(Process process, params int[] offsets) {
        if (offsets.Length == 1) {
            ...
            return IntPtr.Add((IntPtr)new DeepPointer(offsets.First(), [.. offsets.Skip(1).Take(offsets.Length - 2)]).Deref<int>(process), offsets.Last());
        }
